...
# ══════════════════════════════════════════════════════════════════
# MOTEUR ADAPTATIF — PROGRESSION EXERCICES
# ══════════════════════════════════════════════════════════════════

def get_progression_exercices(eleve_id, theme, chapitre, niveau):
    """Récupère l'état de progression adaptatif pour un élève/chapitre/niveau."""
    try:
        res = requests.get(
            f"{supabase_url()}/rest/v1/progression_exercices",
            headers=headers(),
...
def init_progression_exercices(eleve_id, theme, chapitre, niveau):
    """Initialise une nouvelle progression pour un élève/chapitre/niveau."""
    try:
        h = headers(prefer="resolution=merge-duplicates,return=representation")
        requests.post(
            f"{supabase_url()}/rest/v1/progression_exercices",
            ...
            json={
                ...
                "exercices_tentes":   0,
                "exercices_reussis":  0,
                "serie_actuelle":     "normale",
                "rattrapage_tentes":  0,
                "rattrapage_reussis": 0,
                "niveau_valide":      False
            }
        )
    except Exception:
...
def maj_progression_exercices(eleve_id, theme, chapitre, niveau, reussi, serie):
    """Met à jour la progression après chaque exercice."""
    try:
        prog = get_progression_exercices(eleve_id, theme, chapitre, niveau)
        if not prog:
            init_progression_exercices(eleve_id, theme, chapitre, niveau)
            prog = {
                "exercices_tentes":   0,
                "exercices_reussis":  0,
                "rattrapage_tentes":  0,
        ...
        if serie == "normale":
            updates = {
                "exercices_tentes":  prog["exercices_tentes"] + 1,
                "exercices_reussis": prog["exercices_reussis"] + (1 if reussi else 0)
            }
        else:
            updates = {
                "rattrapage_tentes":  prog["rattrapage_tentes"] + 1,
                "rattrapage_reussis": prog["rattrapage_reussis"] + (1 if reussi else 0)
            }
...
def valider_niveau(eleve_id, theme, chapitre, niveau):
    """Marque un niveau comme validé."""
    try:
        requests.patch(
            f"{supabase_url()}/rest/v1/progression_exercices",
            headers=headers(),
            ...
            json={"niveau_valide": True}
        )
    except Exception:
        pass

def reset_rattrapage(eleve_id, theme, chapitre, niveau):
    """Remet à zéro le rattrapage pour une nouvelle série."""
    try:
        requests.patch(
            f"{supabase_url()}/rest/v1/progression_exercices",
            headers=headers(),
            ...
            json={
                "rattrapage_tentes":  0,
                "rattrapage_reussis": 0,
                "serie_actuelle":     "rattrapage"
            }
        )
    except Exception:
        pass

def sauvegarder_erreur(eleve_id, theme, chapitre, niveau,
                       type_ex, question, reponse_eleve, solution):
    """Sauvegarde une erreur pour cibler le rattrapage."""
    try:
        requests.post(
            f"{supabase_url()}/rest/v1/historique_erreurs",
            headers=headers(),
            json={
                "eleve_id":      eleve_id,
                "theme":         theme,
...
def get_erreurs_recentes(eleve_id, theme, chapitre, niveau, limite=3):
    """Récupère les dernières erreurs pour générer le rattrapage ciblé."""
    try:
        res = requests.get(
            f"{supabase_url()}/rest/v1/historique_erreurs",
            headers=headers(),
            params={
                "select":   "type_ex,question,reponse_eleve,solution",
                "eleve_id": f"eq.{eleve_id}",
                "theme":    f"eq.{theme}",
                "chapitre": f"eq.{chapitre}",
                "niveau":   f"eq.{niveau}",
